...
class GMSDRSupervisor:
    ...
    def evaluateTest(self, dataset='test', batches_seen=0):
        with torch.no_grad():
            ...
            y_truth = torch.stack(y_truth,dim=1)
            y_preds = torch.stack(y_preds,dim=1)
            mae, rmse, mape = self._print_loss(y_truth, y_preds)
    ...
    def _train(self, base_lr,
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            if val_loss < min_val_loss:
                ...
                mae, rmse, mape = self.evaluateTest(dataset='test', batches_seen=batches_seen)
                message = 'Interval Test: Epoch [{}/{}] ({}) mae: {:.4f}, rmse: {:.4f},  mape: {:.6f}, ' \
                          '{:.1f}s'.format(epoch_num, epochs, batches_seen,
    ...
    def _print_loss(self, y_true, y_predicted):
        y_true = self.standard_scaler.inverse_transform(y_true)
        y_predicted = self.standard_scaler.inverse_transform(y_predicted)
        mae = masked_mae_loss(y_predicted, y_true)
        rmse = masked_rmse_loss(y_predicted, y_true)
        mape = masked_mape_loss(y_predicted, y_true)
        return mae, rmse, mape
